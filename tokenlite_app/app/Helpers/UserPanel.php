...
class UserPanel
{
    ...
    public static function user_balance_card($data = null, $atttr = '')
    {
        ...
        <span class="lead">' . to_num_token($user->tokenBalance, 2) . ' <span>' . token('symbol') . ' <em class="fas fa-info-circle fs-11" data-toggle="tooltip" data-placement="right" title="' . __('Equivalent to') . ' ' . token_price($user->tokenBalance, base_currency()) . ' ' . base_currency(true) . '"></em></span></span>
        </div>
     ...
     * @return void
     */
    public static function user_token_block($data = '', $atttr = '', $notReady = true)
    {
        return "";
        ...
        if ($vers == 'buy') {
            ...
            if ($notReady == true) {
                //$return .= 'Token Sale soon!';
            } else {
                $return .= '<a href="' . route('user.token') . '" class="btn btn-md btn-primary">' . __('Buy Token Now') . '</a>';
            }
            $return .= '</div>';
    ...
    public static function user_kyc_info($data = null, $atttr = '')
    {
        ...
        if (!isset($user->kyc_info->status)) {
            $ukyc .= '<p class="lead text-light pdb-0-5x">' . __('You have not submitted your documents to verify your identity (KYC).') . '</p><a href="' . route('user.kyc') . '" class="btn btn-sm m-2 btn-icon btn-primary">' . __('Click to Proceed') . '</a>';
        ...
        if (isset($user->kyc_info->status) && $user->kyc_info->status == 'approved') {
            $ukyc .= '<p class="lead text-success pdb-0-5x"><strong>' . __('Identity (KYC) has been verified.') . '</strong></p><p>' . __('One for our team verified your identity. You are eligible to participate in our token sale.');
    ...
    public static function content_block($data = null, $atttr = '')
    {
        ...
        if ($data == 'welcome') {
            $return .= '<style type="text/css">.content-welcome-block > .card-innr { background: #000; }</style>';
